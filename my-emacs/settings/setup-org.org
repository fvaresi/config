...
** General Settings
   ...
   Enable exporting org files to slack.
...
** Org Capture
   ...
   #+BEGIN_SRC emacs-lisp
	...
	(mapcar #'(lambda (c) (if (equal c ?[) ?\( (if (equal c ?]) ?\) c))) string-to-transform)))
     ...
     (defun strip-square-brackets(string-to-transform)
       "Transforms [ into ( and ] into ), other chars left unchanged."
       (concat
	(delq nil
	      (mapcar #'(lambda (c) (if (equal c ?[) nil (if (equal c ?]) nil c))) string-to-transform))))

     (setq org-capture-templates
	   `(;; ("L" "JIRA" entry
	     ;;  (file+headline "~/Documents/baxenergy/notes.org" "Tasks")
	     ;;  "* TODO [[%:link][%(strip-square-brackets \"%:description\")]]")
	     ...
	     ("m" "Meeting" entry
	      (file+headline "~/Documents/baxenergy/notes.org" "Meetings")
	      "* %?"
	      :clock-in t
	      :kill-buffer t)

	     ;; ("j" "Journal Entry" entry
	     ;;  (file+datetree default-journal-file)
	     ;;  "* %?"
	     ;;  :empty-lines 1)
	     ))
   #+END_SRC
