...
*** Popwin
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (push '("*helm ag results*" :height 0.3) popwin:special-display-config)
...
*** All languages
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (defun string-inflection-mind-cycle-function (str)
	"foo_bar => FOO_BAR => fooBar => foo-bar => Foo_Bar => foo_bar
	 foo     => FOO     => Foo    => foo"
	(cond
	 ;; foo => FOO
	 ...
	 (t
      ...
      (defun string-inflection-mind-cycle ()
    ...
    Do not use truncate lines by default.
...
*** Clojure
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package cider
	:ensure t
	:config (setq cider-repl-history-file "~/.emacs.d/cider-repl-history"))
    #+END_SRC
...
*** PHP
	...
	(setq flycheck-php-phpcs-executable "~/.config/composer/vendor/bin/phpcs")
        ...
        (define-key hs-minor-mode-map (kbd "C-c C-o") 'hs-toggle-hiding)
...
*** Window defuns
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (use-package avy
        :ensure t
        :bind ("C-o" ("Jump to word" . avy-goto-word-or-subword-1)))
...
** Version Control
   ...
   #+begin_src emacs-lisp
     (setq magit-repository-directories '(("~/projects/web-app" . 0)
					  ("~/projects/mind-api-client-library" . 0)
					  ("~/projects/mind-aggregation-tier" . 0)
					  ("~/projects/mind-entity" . 0)
					  ("~/projects/mind-experience" . 0)
					  ("~/projects/mind-experience-progress" . 0)
					  ("~/projects/mind-reporting" . 0)
					  ("~/projects/mind-package-api" . 0)
					  ("~/projects/mind-package-clients" . 0)
					  ("~/projects/mind-package-identity" . 0)
					  ("~/projects/mind-package-mcache" . 0)
					  ("~/projects/mind-package-mlog" . 0)
					  ("~/projects/mind-package-model" . 0)
					  ("~/projects/mind-package-build" . 0)
					  ("~/projects/mind-package-test" . 0)))
   #+end_src
...
** Qubes
   ...
   #+BEGIN_SRC emacs-lisp
     ;; (add-to-list 'load-path "~/my-projects/qubes.el")
     ...
     ;;   '(("^https:.*[.]mindresearch[.]org.*" . "current")
     ;;     ("^file:.*" . "current")))
...
** Mind
   ...
   Visit ticket in current line or region.

   #+begin_src emacs-lisp
     (defun mind/open-jira-ticket ()
       "Visit JIRA ticket in line"
       (interactive)

       (let* ((regex "\\(?:ADMIN\\|APP\\|CORELSN\\|DBT\\|ED\\|ID\\|LEARN\\|PLAT\\|STP\\|TRIAG\\|TUX\\)-[[:digit:]]+")
	      (current (if (use-region-p)
			   (buffer-substring-no-properties (region-beginning) (region-end))
			   (thing-at-point 'line t)))
	      (matches (delete-dups (sort (mapcar 'car
						  (s-match-strings-all regex current))
					  'string-lessp))))
	 (when matches
	   (let ((tickets (if (> (length matches) 1)
			      (let ((selected (completing-read "Which ticket?" (cons "All" matches))))
				(if (string-equal "All" selected)
				    matches
				  `(,selected)))
			    matches)))
	     (dolist (ticket tickets)
	       (browse-url (concat "https://jira.mindresearch.org/browse/" ticket)))))
	 ))
     (global-set-key (kbd "C-S-j") 'mind/open-jira-ticket)
   #+end_src
